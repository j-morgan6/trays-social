...
/// FAB on the left and the user's Profile avatar on the right.
/// Settings is intentionally absent (it lives inside Profile).
///
...
/// per HIG. D76 standardized the button sizes; D92 dropped the inner
/// `.frame(maxWidth: .infinity)` so the pill chrome hugs its two-
/// button content instead of stretching the full screen width and
/// reading as a mostly-empty bar. The enclosing VStack in
/// AppShellView centers the now content-sized pill horizontally.
